...
public class RSAUtil {
    ...
    public static String sign(String prikeyvalue, String sign_str, String password) {
        try {
            byte[] bytesBASE64 = Base64Util.getBytesBASE64(prikeyvalue);
            KeyStore ks = KeyStore.getInstance("PKCS12");
            InputStream steam = new ByteArrayInputStream(bytesBASE64);
            ks.load(steam, password.toCharArray());
            Enumeration<String> aliase = ks.aliases();
            String keyAliase = null;
            if (aliase.hasMoreElements()) {
                keyAliase = aliase.nextElement();
            }
            PrivateKey myprikey = (PrivateKey) ks.getKey(keyAliase, password.toCharArray());
            Signature signet = Signature.getInstance("MD5withRSA");
            signet.initSign(myprikey);
            signet.update(sign_str.getBytes("UTF-8"));
            byte[] signed = signet.sign();
